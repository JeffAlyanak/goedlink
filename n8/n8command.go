...
func (n8 *N8) GetConfig() *MapConfig {
	...
	data := make([]uint8, len(cfg.GetBinary()))

	n8.ReadMemory(ADDR_CFG, data, (uint32)(len(cfg.GetBinary())))
...
func getTestMapper(mapper uint16) string {
	home := "./" // TODO: take this as user input

	maprout, _ := os.ReadFile(home + "MAPROUT.BIN") // TODO: error checking
	...
	return mapPath + string(pack) + "/output_files/top.rbf"
}
...
// LoadNewGame loads a new game on the N8.
//
// Creates a `usb_games` directory for USB games and writes the ROM
// and optional mapper `*.RBF` to it. It then selects the game and
// runs it.
func (n8 *N8) LoadNewGame(romPath string, mapPath string) {
	directory := "sd:usb_games"

	n8.MakeDir(directory)

	romDestinationPath := directory + "/" + romPath
	fileData, _ := os.ReadFile(romPath) // TODO: error checking

	n8.OpenFile(romPath, FAT_WRITE|FAT_CREATE_ALWAYS)
...
func (n8 *N8) fpgaPostInit(config *MapConfig) {
	...
	if config != nil {
		config.GetBinary()
		n8.WriteMemory(ADDR_CFG, config.GetBinary(), (uint32)(len(config.GetBinary())))
	}
}
